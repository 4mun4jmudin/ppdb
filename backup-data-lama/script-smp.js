...
const scriptURL_SMP = 'https://script.google.com/macros/s/AKfycbwsjkiTpxJ8bW3vcu8sCYOribsk8mIviNoCcWM-1a4kuFSFOTnd6Z3sS3tielH3fFfX/exec';
...
form.addEventListener('submit', async (e) => {
  ...
  if (!scriptURL_SMP || scriptURL_SMP.startsWith('GANTI')) {
    showToast('❌ URL Google Apps Script SMP belum dikonfigurasi di script-smp.js', 'error');
    return;
  }
  ...
  try {
    ...
    // Override pekerjaan/alasan "lainnya"
    document.querySelectorAll('.select-pekerjaan, .select-lainnya').forEach(sel => {
      if (sel.value === 'lainnya') {
        const inp = sel.parentElement.querySelector('.input-pekerjaan-lainnya, .input-lainnya-text');
        if (inp?.value.trim()) formData.set(sel.name, inp.value.trim());
      }
    });

    const fileKeys = ['fileIjazah', 'fileSHUSM', 'filePhoto', 'fileKK', 'fileKTP', 'fileAkta', 'fileKIP'];
    const dataToSend = {};
    formData.forEach((val, key) => {
      if (!fileKeys.includes(key)) dataToSend[key] = val;
    });

    showToast('⏳ Memproses file lampiran, harap tunggu...', '');

    const [fileIjazah, fileSHUSM, filePhoto, fileKK, fileKTP, fileAkta, fileKIP] = await Promise.all(
      fileKeys.map(id => getFileBase64(id))
    );
    Object.assign(dataToSend, { fileIjazah, fileSHUSM, filePhoto, fileKK, fileKTP, fileAkta, fileKIP });

    await fetch(scriptURL_SMP, {
      method: 'POST',
    ...
    showNotification();
    localStorage.removeItem('ppdb_smp_draft');
    localStorage.removeItem('ppdb_smp_step');
    form.reset();
    document.querySelectorAll('.file-upload-name').forEach(el => { el.textContent = 'Belum ada file dipilih'; el.style.color = ''; });
    document.querySelectorAll('.file-upload-label').forEach(el => el.style.borderColor = '');
    goToStep(1);

  } catch (err) {
    console.error('Submit error:', err);
    showToast(err.message || 'Gagal mengirim. Periksa koneksi internet.', 'error');
  } finally {
